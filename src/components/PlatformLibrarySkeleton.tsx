...
import type { Platform } from "../types/platformOperations";
import { getPlatformPhaseLabel } from "../utils/platformOperationReducer";

interface PlatformLibrarySkeletonProps {
  platform: Platform;
...
export const PlatformLibrarySkeleton: React.FC<PlatformLibrarySkeletonProps> = ({
  platform,
  phase,
  completed,
  total,
  ...
  const phaseLabel = getPlatformPhaseLabel(phase);
  const progressText =
  ...
  return (
    <div
      ...
      aria-live="polite"
      aria-busy="true"
      className="w-full flex-1 flex flex-col justify-between overflow-hidden select-none pointer-events-none"
    >
            ...
            >
              {skeletonCards.map((offset) => {
                ...
                return (
                  ...
                  >
                    <div
                      data-testid="platform-game-skeleton"
